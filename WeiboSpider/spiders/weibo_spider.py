...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    def parse_follow(self, response):
        ...
        for node in node_l:
            if node.xpath('./a[1]/text()').extract():
                ...
                nodes_i = re.findall('uid=(\d+)', ";".join(url), re.S)[0]
    ...
    def parse_fans(self, response):
        ...
        # nodes = re.findall('uid=(\d+)', ";".join(urls), re.S)
        # fans = [{'id': node} for node in nodes]
        ...
        for node in node_l:
            if node.xpath('./a[1]/text()').extract():
                ...
                url = node.xpath('.//a[text()="关注他" or text()="关注她" or text()="移除"]/@href').extract()
                nodes_i = re.findall('uid=(\d+)', ";".join(url), re.S)[0]
                nodes_n = node.xpath('./a[1]/text()').extract()[0]

                fans_date = {'name': nodes_n, 'id': nodes_i}
    ...
    def parse_comment(self, response):
        for comment_node in response.xpath('//div[@class="c" and contains(@id,"C_")]'):
            ...
            comment_item['id'] = comment_node.xpath('./@id').extract()[0]
            ...
            comment_item['created_at'] = created_at_info.split('\xa0')[0]
            yield comment_item
